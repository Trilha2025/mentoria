...
import { ClipboardDocumentListIcon, TrashIcon, CheckIcon, CalendarDaysIcon } from '@heroicons/react/24/outline';
...
export default function StudyPlanPage() {
    ...
    const handleAddToAgenda = (lessonId: string, lessonTitle: string) => {
        // Redirect to new Agenda page with pre-filled event info
        window.location.href = `/agenda?lessonId=${lessonId}&title=${encodeURIComponent(`Estudo: ${lessonTitle}`)}`;
    };
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs mt-1">
                                                        <span className="text-trenchy-text-secondary">Adicionado em {new Date(lesson.addedAt).toLocaleDateString()}</span>
                                                        {lesson.scheduledAt && (
                                                            <span className="flex items-center gap-1.5 text-trenchy-orange font-medium bg-trenchy-orange/10 px-2 py-0.5 rounded-full border border-trenchy-orange/20">
                                                                <CalendarDaysIcon className="w-3 h-3" />
                                                                Agendado para {new Date(lesson.scheduledAt).toLocaleDateString()} às {new Date(lesson.scheduledAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                            </span>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button
                                                        onClick={() => handleAddToAgenda(lesson.id, lesson.title)}
