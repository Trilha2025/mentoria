...
export const ModuleCard = ({ id, title, status, description }: ModuleProps) => {
    ...
    return (
        <div 
            className={`p-6 rounded-xl border border-trenchy-border transition-all flex flex-col justify-between h-full ${isLocked ? 'bg-black/5 dark:bg-white/5 opacity-50 cursor-not-allowed' : 'bg-trenchy-card shadow-lg hover:shadow-orange-900/10 hover:border-trenchy-orange/30'
            }`}>
            <div>
            ...
            {!isLocked ? (
                ...
                <div className="w-full py-3 bg-black/5 dark:bg-white/5 text-gray-400 text-center rounded-lg text-xs font-bold">
                    CONTEÚDO BLOQUEADO
                </div>
            )}
        </div>
